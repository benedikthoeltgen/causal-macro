...
# load activations
X_activ = joblib.load('results/Simulated/variables/XtoY'+g+b+'.pkl')
Y_activ = joblib.load('results/Simulated/variables/YtoX'+g+b+'.pkl')
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 9:
            ...
            if val_loss < best_loss:
                torch.save(vae.state_dict(), 'results/Simulated/anm_vae_training/nn_params_'+xory+g+b+n+'.pth')
                best_loss = val_loss
                best_loss_epoch = epoch
    print('best loss: ', round(best_loss,3), ' in epoch ', best_loss_epoch)
    
    #load best loss net and compute outputs
    vae.load_state_dict(torch.load('results/Simulated/anm_vae_training/nn_params_'+xory+g+b+n+'.pth'))
    xy_out, _ = vae(XY)
    x_trans, y_trans, _, _, _ = xy_out
    #save activations etc.
    joblib.dump((x_trans.detach().numpy(),
                 y_trans.detach().numpy()), 'results/Simulated/transformed_variables/trans_xy_'+xory+g+b+n+'.pkl')
...
for neuron in (1,3):
    for xory in ('x', 'y'):
        ...
        joblib.dump((testset,trainset), 'results/Simulated/anm_vae_training/test+trainset_'+xory+g+b+n+'.pkl')
        XY_tr = XY[trainset]
        XY_ts = XY[testset]
        ...
        ### train
        print(n, xory)
        train(2000, conn_weight=1, hsic_weight=1)                                               ### train
        
        ### save losses
        with open('results/Simulated/anm_vae_training/losses_'+xory+g+b+n+'.py', 'wb') as fp:
            pickle.dump(losses, fp)
        with open('results/Simulated/anm_vae_training/losses_'+xory+g+b+n+'_tr.py', 'wb') as fp:
